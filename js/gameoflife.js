...
const willBeAlive = (cell, state) => {
  // Calculate how many living neighbours a cell has
  let living = getLivingNeighbors(cell, state).length // number
  let cell_status = contains.call(state, cell) // True or false

  return ((cell_status && living == 2) || living == 3)
};

const calculateNext = (state) => { 
  const {bottomLeft, topRight} = corners(state)
  // add a step to the row or column in the next state
  let new_state = []
  for(let y=topRight[1]+1; y>=bottomLeft[1]-1; y--){
    for(let x=bottomLeft[0]-1; x<=topRight[0]+1; x++){
      new_state = new_state.concat(willBeAlive([x,y],state) ? [[x,y]]: [])
    }    
  }
  return new_state
};
